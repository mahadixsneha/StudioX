...
import { generateEditedImage, generateFilteredImage, generateAdjustedImage, generateReplacedBackground } from './services/geminiService';
...
type Tab = 'retouch' | 'adjust' | 'background' | 'ai-filters' | 'crop';
...
const App: React.FC = () => {
  ...
  const handleApplyBackground = useCallback(async (backgroundPrompt: string) => {
    ...
    try {
        const newImageUrl = await generateReplacedBackground(currentImage, backgroundPrompt);
        const newImageFile = dataURLtoFile(newImageUrl, `background-${Date.now()}.png`);
        addImageToHistory(newImageFile);
    } catch (err) {
        const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred.';
        setError(`Failed to replace the background. ${errorMessage}`);
        console.error(err);
    } finally {
        setIsLoading(false);
    }
      ...
      <div className="w-full max-w-4xl mx-auto flex flex-col items-center gap-6 animate-fade-in">
        ...
        <div className="w-full bg-gray-800/80 border border-gray-700/80 rounded-lg p-2 flex items-center justify-center gap-2 backdrop-blur-sm">
            {([
                ...
                { id: 'ai-filters', label: 'AI Filters' },
        ...
        <div className="w-full">
            ...
            {activeTab === 'ai-filters' && <FilterPanel onApplyFilter={handleApplyFilter} isLoading={isLoading} />}
        </div>
